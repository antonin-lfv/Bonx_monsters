...
BLP_auth = Blueprint(
    "BLP_auth", __name__, template_folder="templates", static_folder="static"
)
...
def login():
    if request.method == "POST":
        if (email := request.form.get("login_email")) and (
...
def registration():
    if request.method == "POST":
        if (
            ...
            and (password := request.form.get("signup_password"))
            and (name := request.form.get("signup_name"))
        ):
            user = User.query.filter_by(email=email).first()
            # if user already exists
            if user:
            ...
            monsters_random_name = random.sample(
                sorted(monsters), GameConfig.USER_STARTING_NUMBER_OF_MONSTERS
            )
